  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
        <h2 style={{ fontFamily: 'Inter, sans-serif', fontSize: '1.1rem', fontWeight: 700, color: 'var(--text-1)', margin: '0 0 4px', letterSpacing: '-0.02em' }}>
          AI CFO Assistant
        </h2>
      ...
      <div ref={scrollRef} onScroll={onBodyScroll} role="log" aria-live="polite" aria-atomic="false" aria-label="Conversation with AI CFO"
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
          ...
          <div>
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.88rem', fontWeight: 700, color: 'var(--text-1)', margin: 0 }}>AI CFO</p>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {QUICK_PROMPTS.map((prompt) => (
                <button key={prompt} type="button" onClick={() => submit(prompt)} disabled={loading}
                  style={{ padding: '6px 12px', borderRadius: 20, border: '1px solid var(--border-md)', background: 'var(--bg-badge)', cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.62rem', color: 'var(--text-2)', opacity: loading ? 0.5 : 1, transition: 'all 0.15s ease', whiteSpace: 'nowrap' }}
          ...
          {messages.map((msg) => (
            <motion.div key={msg.id} initial={{ opacity: 0, y: 8, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.25, ease: 'easeOut' }}
              ...
              <div style={{
                ...
                <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.82rem', color: msg.role === 'user' ? '#fff' : 'var(--text-2)', lineHeight: 1.55, margin: '0 0 6px', whiteSpace: 'pre-wrap' }}>
                  {msg.role === 'assistant' ? plain(msg.content) : msg.content}
                </p>
                <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.6rem', color: msg.role === 'user' ? 'rgba(255,255,255,0.55)' : 'var(--text-4)', margin: 0, textAlign: msg.role === 'user' ? 'right' : 'left' }}>
                  {msg.timestamp}
                </p>
              </div>
            </motion.div>
      ...
      <div style={{ padding: '12px 16px 16px', borderTop: '1px solid var(--border)', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10, background: 'var(--bg-badge)', border: '1px solid var(--border-md)', borderRadius: 12, padding: '10px 12px 10px 16px', transition: 'border-color 0.15s ease' }}>
          ...
          <textarea
            ...
            style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', resize: 'none', fontFamily: 'Inter, sans-serif', fontSize: '0.82rem', color: 'var(--text-1)', lineHeight: 1.5, maxHeight: 120, overflow: 'auto' }}
        ...
        <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.6rem', color: 'var(--text-4)', textAlign: 'center', margin: '8px 0 0' }}>
