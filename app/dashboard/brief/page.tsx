...
function RecommendationCard({
  title, recommendation, priority, index,
}: {
  title: string; recommendation: string; priority: string; index: number;
}) {
  const priorityColour = priority === 'high' || priority === 'critical'
    ? 'var(--crit)'
    : priority === 'medium'
    ? 'var(--warn)'
    : 'var(--good)';
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
          ...
          <span className="badge" style={{
            ...
            background: `color-mix(in srgb, ${priorityColour} 10%, transparent)`,
            borderColor: `color-mix(in srgb, ${priorityColour} 25%, transparent)`,
          }}>
        ...
        <p style={{
          ...
          color: 'var(--text-3)', lineHeight: 1.55, margin: 0,
        }}>
          {recommendation}
        </p>
      </div>
...
export default function BriefPage() {
  ...
  if (alerts.filter((a: any) => a.severity === 'warning' || a.severity === 'critical').length > 0) {
    ...
    <>
      ...
      <div className="grid-kpi" style={{ marginBottom: 24 }}>
        ...
        <KPICard
          ...
          sparkColor="var(--good)" delay={0.06}
        />
        <KPICard
          label="NET PROFIT" value={fmt(kpi.totalProfit, true, sym)}
          sub={`${kpi.avgMargin.toFixed(1)}% avg margin`}
          growth={12.1}
          ...
          sparkData={monthly.slice(-6).map(m => (Number(m.Revenue) || 0) - (Number(m.Costs) || 0))}
          sparkColor="var(--cyan)" delay={0.12}
        />
        <KPICard
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: 28, alignItems: 'center' }}>
          ...
          <div style={{ position: 'relative', width: 130, height: 130 }}>
            <svg width="130" height="130" viewBox="0 0 130 130">
              ...
              <motion.circle
                cx="65" cy="65" r="52"
                fill="none" stroke={healthColour} strokeWidth="10"
                strokeLinecap="round"
          ...
          <div>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div className="progress-track">
                <motion.div className="progress-fill" style={{ background: 'var(--good)', width: '100%' }}
                  initial={{ width: 0 }} animate={{ width: '100%' }} transition={{ duration: 1, ease: 'easeOut', delay: 0.4 }} />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="progress-track">
                <motion.div className="progress-fill" style={{ background: 'var(--warn)' }}
                  initial={{ width: 0 }} animate={{ width: '65%' }} transition={{ duration: 1, ease: 'easeOut', delay: 0.5 }} />
              </div>
            </div>
            ...
            {scores && (
              <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
                {[
                  { label: 'Financial',    score: scores.e1_score, colour: 'var(--e1)' },
                  { label: 'Customer',     score: scores.e2_score, colour: 'var(--e2)' },
      ...
      {briefLines.length > 0 && (
        <SectionCard title="Executive Action Plan" subtitle="AI-BOS Intelligence · E1 + Customer Intelligence + Operations" delay={0.24} style={{ marginBottom: 20 }}>
          {briefLines.map((line, i) => (
            <BriefPoint key={i} text={line} index={i} colour="var(--cyan)" />
          ))}
        </SectionCard>
      )}
