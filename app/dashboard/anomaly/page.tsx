...
// ── Anomaly Stat Card (per-color personality) ─────────────────────────────────
function AnomalyStatCard({
  label, value, suffix, colour, glow, delay = 0, pulse = false,
}: {
  ...
  return (
    <motion.div
      ...
      animate={pulse
        ? { opacity: 1, y: 0, boxShadow: [`0 4px 32px rgba(0,0,0,0.4), 0 0 0px ${colour}00`, `0 4px 32px rgba(0,0,0,0.4), 0 0 22px ${colour}30`, `0 4px 32px rgba(0,0,0,0.4), 0 0 0px ${colour}00`] }
        : { opacity: 1, y: 0 }}
      transition={pulse
        ? { opacity: { duration: 0.4, delay }, y: { duration: 0.4, delay }, boxShadow: { duration: 2.4, delay: delay + 0.4, repeat: Infinity, ease: 'easeInOut' } }
        : { duration: 0.4, delay }}
      style={{
    ...
    >
      {/* Glow orb */}
      <div style={{ position: 'absolute', top: -28, right: -28, width: 96, height: 96, borderRadius: '50%', background: `radial-gradient(circle, ${glow}, transparent 70%)`, pointerEvents: 'none' }} />
      ...
      <p style={{ fontSize: '2rem', fontWeight: 700, fontFamily: 'Outfit, sans-serif', color: colour, margin: 0, letterSpacing: '-0.03em', lineHeight: 1 }}>
        {value}
        <span style={{ fontSize: '1rem', fontWeight: 400, fontFamily: 'DM Mono, monospace', marginLeft: 4 }}>{suffix}</span>
      </p>
    </motion.div>
  );
}
    ...
    <div style={{ width: '100%' }}>
      ...
      <div style={{ height: 5, background: 'rgba(99,179,237,0.07)', borderRadius: 999, overflow: 'hidden', position: 'relative' }}>
        ...
        <motion.div
          ...
          style={{ height: '100%', borderRadius: 999, background: colour, boxShadow: `0 0 6px ${colour}80` }}
        />
      </div>
...
function AnomalyTooltip({ active, payload, sym }: any) {
  ...
  return (
    <div style={{
      ...
      border:      `1px solid ${d.colour}40`,
      borderLeft:  `3px solid ${d.colour}`,
      borderRadius:'0 12px 12px 0',
      padding:     '12px 16px',
      boxShadow:   `0 12px 40px rgba(0,0,0,0.6), 0 0 20px ${d.colour}12`,
    }}>
              ...
              return (
                <motion.div
                  ...
                  style={{
                    ...
                    borderRadius:   '0 14px 14px 0',
                    padding:        '20px',
                    boxShadow:      `0 4px 28px rgba(0,0,0,0.35), inset 40px 0 60px ${cfg.bg}`,
                    position:       'relative',
                    overflow:       'hidden',
                  }}
                >
                  {/* Severity glow orb */}
                  <div style={{ position: 'absolute', top: -20, right: -20, width: 80, height: 80, borderRadius: '50%', background: `radial-gradient(circle, ${cfg.colour}18, transparent 70%)`, pointerEvents: 'none' }} />
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
                    <div style={{ background: `${cfg.colour}08`, border: `1px solid ${cfg.colour}18`, borderRadius: 8, padding: '10px 12px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12, padding: '6px 0', borderTop: '1px solid rgba(99,179,237,0.06)' }}>
                    <span style={{ fontSize: '0.66rem', fontFamily: 'DM Mono, monospace', color: '#4a6285' }}>Deviation from expected</span>
      ...
      <Card delay={0.2}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
          <div>
            ...
            <p style={{ fontSize: '0.66rem', color: '#4a6285', fontFamily: 'DM Mono, monospace', margin: 0 }}>
              Monthly cost values plotted · σ thresholds shown
            </p>
          </div>
          <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
            {[['#60A5FA','Normal'],['#F59E0B','≥2σ High'],['#EF4444','≥3σ Critical']].map(([c, l]) => (
        ...
        <ResponsiveContainer width="100%" height={240}>
          <ScatterChart margin={{ top: 8, right: 8, bottom: 16, left: -4 }}>
            <CartesianGrid strokeDasharray="4 4" stroke="rgba(99,179,237,0.05)" />
            <XAxis
              dataKey="x"
              {...AXIS}
              label={{ value: 'Month Index', fill: '#2d4a70', fontSize: 10, fontFamily: 'DM Mono, monospace', position: 'insideBottom', offset: -8 }}
            />
            <YAxis
              dataKey="y"
              {...AXIS}
            ...
            <Tooltip
              ...
              content={<AnomalyTooltip sym={sym} />}
            />
            <ReferenceLine y={0} stroke="transparent" />
            {/* σ band lines */}
            <ReferenceLine
              y={scatterData.length ? (monthly.reduce((s,m) => s+m.costs,0)/monthly.length + 2 * Math.sqrt(monthly.reduce((s,m) => s+Math.pow(m.costs - monthly.reduce((a,b)=>a+b.costs,0)/monthly.length,2),0)/monthly.length)) : 130000}
              stroke="rgba(245,158,11,0.35)"
              strokeDasharray="6 3"
              label={{ value: '+2σ', fill: '#f59e0b', fontSize: 9, fontFamily: 'DM Mono, monospace', position: 'insideTopRight' }}
            />
            <ReferenceLine
              y={scatterData.length ? (monthly.reduce((s,m) => s+m.costs,0)/monthly.length + 3 * Math.sqrt(monthly.reduce((s,m) => s+Math.pow(m.costs - monthly.reduce((a,b)=>a+b.costs,0)/monthly.length,2),0)/monthly.length)) : 140000}
            ...
            <Scatter
              data={scatterData}
              line={{ stroke: 'rgba(99,179,237,0.15)', strokeWidth: 1.5 }}
              lineType="joint"
            >
              ...
              {anomalies.map((a, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: '10px 12px' }}>
                      <span style={{ fontSize: '0.6rem', fontFamily: 'DM Mono, monospace', color: cfg.colour, background: cfg.bg, border: `1px solid ${cfg.border}`, padding: '2px 9px', borderRadius: 4 }}>
                        {cfg.label}
                      </span>
                    </td>
                  </motion.tr>
