      ...
      {(phase === 'idle' || phase === 'parsing') && (
        <SectionCard title="Choose a file" subtitle="Excel (.xlsx/.xls) or CSV — any layout">
          <input
            ...
            style={{ display: 'block', width: '100%', padding: '20px', minHeight: 56, border: '1px dashed var(--upload-border)', borderRadius: 10, background: 'var(--upload-bg)', color: 'var(--text-2)', fontFamily: 'Inter, sans-serif', fontSize: '0.85rem', cursor: 'pointer' }}
          />
          ...
          <SectionCard title="Preview" subtitle={`${validCount} of ${preview.rows.length} sample rows look valid`}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 16, flexWrap: 'wrap' }}>
              <button type="button" onClick={commit} disabled={phase === 'committing'} className="touch-target"
                style={{ padding: '10px 20px', minHeight: 44, borderRadius: 10, border: 'none', background: 'var(--green)', color: '#04140d', fontFamily: 'Inter, sans-serif', fontSize: '0.875rem', fontWeight: 700, cursor: 'pointer', opacity: phase === 'committing' ? 0.7 : 1 }}>
                {phase === 'committing' ? 'Importing…' : `Import ${preview.row_count} rows`}
              </button>
              <button type="button" onClick={reset} className="touch-target"
                style={{ padding: '10px 20px', minHeight: 44, borderRadius: 10, border: '1px solid var(--border-md)', background: 'transparent', color: 'var(--text-2)', fontFamily: 'Inter, sans-serif', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer' }}>
                Choose another file
              </button>
            </div>
          </SectionCard>
        ...
        <SectionCard title="Import complete" subtitle="Your dashboards have been updated">
          ...
          {result.error_count > 0 && (
            <details style={{ marginBottom: 16 }}>
              <summary style={{ cursor: 'pointer', fontFamily: 'Inter, sans-serif', fontSize: '0.8rem', color: 'var(--text-3)' }}>
                Why were {result.error_count} rows skipped?
              </summary>
              <ul style={{ margin: '8px 0 0', paddingLeft: 18, color: 'var(--text-3)', fontSize: '0.78rem', fontFamily: 'JetBrains Mono, monospace' }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <a href="/dashboard/timeline" style={{ padding: '10px 20px', minHeight: 44, display: 'inline-flex', alignItems: 'center', borderRadius: 10, background: 'var(--cyan)', color: '#04121a', fontFamily: 'Inter, sans-serif', fontSize: '0.875rem', fontWeight: 700, textDecoration: 'none' }}>
              View timeline →
            </a>
            <button type="button" onClick={reset} className="touch-target"
              style={{ padding: '10px 20px', minHeight: 44, borderRadius: 10, border: '1px solid var(--border-md)', background: 'transparent', color: 'var(--text-2)', fontFamily: 'Inter, sans-serif', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer' }}>
