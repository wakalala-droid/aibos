...
export default function AdminAccountsPage() {
  ...
  const header = (
    <header style={{ marginBottom: 18 }}>
      ...
      <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.85rem', color: 'var(--text-3)', margin: 0 }}>
  ...
  const controls = (
    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 16 }}>
      <input
        ...
        style={{ flex: '1 1 220px', minHeight: 42, padding: '9px 14px', borderRadius: 10, border: '1px solid var(--border-md)', background: 'var(--bg-input)', color: 'var(--text-1)', fontFamily: 'Inter, sans-serif', fontSize: '0.84rem', outline: 'none' }}
      />
      <select aria-label="Filter by tier" value={tierFilter} onChange={(e) => setTierFilter(e.target.value as 'all' | Tier)} style={{ minHeight: 42, padding: '9px 12px', borderRadius: 10, border: '1px solid var(--border-md)', background: 'var(--bg-input)', color: 'var(--text-1)', fontFamily: 'Inter, sans-serif', fontSize: '0.82rem' }}>
        <option value="all">All tiers</option>
        <option value="free">Free</option>
        <option value="pro">Pro</option>
        <option value="growth">Growth</option>
      </select>
      <select aria-label="Sort accounts" value={sort} onChange={(e) => setSort(e.target.value as SortKey)} style={{ minHeight: 42, padding: '9px 12px', borderRadius: 10, border: '1px solid var(--border-md)', background: 'var(--bg-input)', color: 'var(--text-1)', fontFamily: 'Inter, sans-serif', fontSize: '0.82rem' }}>
  ...
  if (loading) {
    ...
    body = (
      <div className="section-card" role="alert" style={{ textAlign: 'center' }}>
        <p style={{ fontFamily: 'Inter, sans-serif', color: 'var(--crit)', margin: '0 0 12px' }}>{error}</p>
    ...
    body = (
      <div className="section-card" style={{ textAlign: 'center', padding: '40px 20px' }}>
        <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.9rem', color: 'var(--text-2)', margin: '0 0 4px', fontWeight: 600 }}>No accounts yet</p>
        <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.82rem', color: 'var(--text-3)', margin: 0 }}>Accounts appear here after their first sign-in.</p>
      </div>
    );
  } else if (rows.length === 0) {
    body = (
      <div className="section-card" style={{ textAlign: 'center', padding: '32px 20px' }}>
        <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.84rem', color: 'var(--text-3)', margin: 0 }}>No accounts match your filters.</p>
      </div>
    );
  } else if (narrow) {
          ...
          <div key={a.id} className="section-card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, marginBottom: 10 }}>
              <div style={{ minWidth: 0 }}>
                <Link href={`/admin/${a.id}`} style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.9rem', fontWeight: 700, color: 'var(--text-1)', textDecoration: 'none', display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {a.business_name || '—'}
                </Link>
                <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.66rem', color: 'var(--text-3)' }}>{a.email}</span>
              </div>
            ...
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.66rem', color: 'var(--text-3)', marginBottom: 12 }}>
            ...
            {rows.map((a) => (
              <tr key={a.id}>
                <td style={{ maxWidth: 240 }}>
                  <Link href={`/admin/${a.id}`} style={{ color: 'var(--text-1)', fontWeight: 700, textDecoration: 'none', display: 'block', fontFamily: 'Inter, sans-serif', fontSize: '0.8rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {a.business_name || '—'}
                  </Link>
  ...
  return (
    <div style={{ padding: '8px 0 48px' }}>
      ...
      <nav aria-label="Admin sections" style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <Link href="/admin" aria-current="page" style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-1)', textDecoration: 'none', padding: '6px 12px', borderRadius: 8, background: 'var(--bg-badge)', border: '1px solid var(--border)' }}>Accounts</Link>
