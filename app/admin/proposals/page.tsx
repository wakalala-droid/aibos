  ...
  const renderCard = (p: Proposal) => {
    ...
    return (
      <SectionCard key={p.id}>
        ...
        <div style={{ display: 'grid', gap: 6, fontFamily: 'JetBrains Mono, monospace', fontSize: '0.68rem', color: 'var(--text-3)', margin: '6px 0 12px', padding: '10px 12px', borderRadius: 8, background: 'var(--bg-badge)', border: '1px solid var(--border-md)' }}>
        ...
        {p.status === 'proposed' && (
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => setStatus(p.id, 'monitoring')} disabled={!passed}
              ...
              style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.78rem', fontWeight: 700, color: passed ? '#04210f' : 'var(--text-4)', background: passed ? 'var(--good)' : 'var(--bg-badge)', border: '1px solid var(--border-md)', borderRadius: 8, padding: '8px 14px', cursor: passed ? 'pointer' : 'not-allowed' }}>
              Approve → 15-day monitor
            </button>
            <button onClick={() => deleteProposal(p.id)}
              style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.78rem', fontWeight: 600, color: 'var(--text-2)', background: 'var(--bg-badge)', border: '1px solid var(--border-strong)', borderRadius: 8, padding: '8px 14px', cursor: 'pointer' }}>
              Reject &amp; delete
            </button>
          </div>
        )}
        ...
        {(p.status === 'monitoring' || p.status === 'approved') && (() => {
          ...
          return (
            <div>
              {p.status === 'monitoring' ? (
                <div role="status" style={{ display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'center', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.64rem', color: 'var(--text-3)', margin: '0 0 10px' }}>
                ...
                <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.64rem', color: 'var(--warn)', margin: '0 0 10px' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {p.status === 'monitoring' && (
                  <button onClick={() => setStatus(p.id, 'stable')} disabled={!eligible}
                    ...
                    style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.78rem', fontWeight: 700, color: eligible ? '#04210f' : 'var(--text-4)', background: eligible ? 'var(--good)' : 'var(--bg-badge)', border: '1px solid var(--border-md)', borderRadius: 8, padding: '8px 14px', cursor: eligible ? 'pointer' : 'not-allowed' }}>
                    Promote to stable
                  </button>
                )}
                <button onClick={() => deleteProposal(p.id)}
